...
export async function InsertGroup({ name, label, type, colour, hasAccount, grades, accountRoles }: DbGroup) {
  ...
  if (!insertedGroup) return true;
	...
	)) as UpsertResult | UpsertResult[];
	
	await conn.commit();
	
	const results = Array.isArray(insertedGrades) ? insertedGrades : [insertedGrades];
	return results.reduce((acc, curr) => acc + curr.affectedRows, 0) > 0;
}
