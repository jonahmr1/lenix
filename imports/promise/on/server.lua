...
function lib.promise.on(endpoint, Function)
  ...
  RegisterNetEvent(("__tr_promise_on:%s"):format(endpoint), function(promiseId, ...)
    ...
    if success then
      TriggerClientEvent(("__tr_promise_await:%s"):format(endpoint), source, promiseId, response)
    else
      TriggerClientEvent(("__tr_promise_await:%s"):format(endpoint), source, promiseId, nil)
