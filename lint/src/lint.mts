...
async function getCoreRules(): Promise<Record<string, RuleInfo[]>> {
	try {
		...
		return {
			Core: Object.keys(eslintJs.configs.all.rules ?? {}).map(n => ({
				fullName: n,
				description: '',
				fixable: false,
				recommended: false,
...
function askRule(
	...
	idx: number,
	total: number,
): [string, string] | 'save' | null {
	const tags = [
		rule.recommended ? tag('recommended', c.green) : null,
		rule.fixable ? tag('fixable', c.yellow) : null,
		rule.requiresTypeChecking ? tag('type-aware', c.red) : null,
	]
		.filter(Boolean)
		.join(' ')
	...
	const a = readLine(pr('  → off/warn/error/skip/save [o/w/e/s/q]: '))
		.trim()
		.toLowerCase()
...
function askSection(label: string, rules: RuleInfo[], state: State) {
	...
	process.stdout.write(
		gray(`  ${rules.length} rules — Enter=skip  q=save & exit\n`),
	)

	const skip = readLine(pr('  Skip entire section? [y/N]: '))
		.trim()
		.toLowerCase()
	if (skip === 'y') {
	...
	if (startAt > 0)
		process.stdout.write(
			gray(`  Resuming from rule ${startAt + 1}/${rules.length}\n`),
		)
...
if (clone) {
	...
	process.stdout.write(
		`${tag('done', c.green)} Written to ${bold(data.outFile)}\n`,
	)
	fs.closeSync(ttyFd)
	process.exit(0)
}
...
if (preset) {
	process.stdout.write(
		`${tag('IMPORTANT', c.red)}: This tool is deprecated, import our rules instead, e.g ${bold('import { lint } from "lenix"')}`,
	)
	const res = await fetch(
		'https://raw.githubusercontent.com/LenixDev/lenix/refs/heads/main/lint/src/lint-preset.json',
	)
	const data = (await res.json()) as CheckpointData
	const config = generateConfig(data.rules)
	fs.writeFileSync('eslint.config.mts', config, 'utf8')
	process.stdout.write(
		`${tag('done', c.green)} Written to ${bold('eslint.config.mts')}\n`,
	)
	process.stdout.write(
		`${tag('notice', c.cyan)} Run: ${bold('npm add -D @eslint/js globals typescript-eslint')}\n`,
	)
	fs.closeSync(ttyFd)
	process.exit(0)
}
...
async function main() {
	...
	process.stdout.write(
		dim('o=off  w=warn  e=error  s/Enter=skip  q=save & exit\n\n'),
	)
	...
	if (checkpoint) {
		process.stdout.write(
			`${tag('found', c.yellow)} Saved progress detected (${Object.keys(checkpoint.rules).length} rules configured).\n`,
		)
	...
	const tsAll = buildPluginRules(
		(tseslint.plugin as unknown as PluginWithRules).rules ?? {},
		'@typescript-eslint',
	)
	askSection(
		'TypeScript',
		tsAll.filter(r => !r.requiresTypeChecking),
		state,
	)
	askSection(
	...
	process.stdout.write(
		dim(`Rules configured: ${Object.keys(state.allRules).length}\n`),
	)
	const hasTs = Object.keys(state.allRules).some(k =>
		k.startsWith('@typescript-eslint/'),
	)
	const hasReact = Object.keys(state.allRules).some(
		k => k.startsWith('react/') || k.startsWith('react-hooks/'),
	)
	...
	if (hasTs) {
		...
		else process.stdout.write(dim('tsconfig.json already exists, skipping.\n'))
	}
