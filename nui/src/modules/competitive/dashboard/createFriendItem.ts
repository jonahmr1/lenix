import { useButton, useDiv } from "@trippler/tr_kit/nui"
import playerDetails from "./playerDetails"
import { triggerNuiCallback } from "@trippler/tr_lib/nui"
import { RemovePlayerFriendship } from "../../../../../shared/types/competitive"
...
export default async (identity: number): Promise<void> => {
  ...
  const friend = useDiv({
    ...
    style: "flex items-center justify-between hover:bg-stone-600 text-sm py-4 px-2"
  })

  useDiv({
    parent: `friend-item-${friendsItems}`,
    style: "w-[75%] flex items-center justify-between",
    content: `
  ...
  const inviteBTN = useButton({
    parent: `friend-item-${friendsItems}`,
    style: "w-[16%] h-[12.5%] hover:bg-blue-500 flex items-center justify-center",
    content: '➕',
    onClick: () => {
      inviteBTN.innerText = '✅'
      setTimeout(() => {
