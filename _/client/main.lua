...
local function isPlayerBoss()
  return lib.callback.await('lenix_bossdesk:server:isPlayerBoss')
end

---@return table <id, name, callsign, rank, status>
local function getAllPlayers()
  return lib.callback.await('lenix_bossdesk:server:getJobPlayersData')
end

local function getJobBudget()
  return lib.callback.await('lenix_bossdesk:server:getJobBudget')
end

local function getTotalPlayers()
  return lib.callback.await('lenix_bossdesk:server:getTotalPlayers')
end

local function getJobOnlinePlayers(players)
  return lib.callback.await('lenix_bossdesk:server:getJobOnlinePlayers', false, players)
end

local function getJobRanks()
  return lib.callback.await('lenix_bossdesk:server:getJobRanks')
end
...
local function isSelfInteraction(citizenid)
  return lib.callback.await('lenix_bossdesk:server:isSelfInteraction', false, citizenid)
end
...
RegisterNUICallback('hirePlayer', function(data, cb)
  ...
  lib.callback.register('lenix_bossdesk:server:hirePlayer', false, function(success, message)
    if success then
  ...
  end, data.citizenid, data.rank, data.callsign)
...
RegisterNUICallback('firePlayer', function(data, cb)
  ...
  lib.callback.register('lenix_bossdesk:server:firePlayer', false, function(success, message)
    if success then
...
RegisterNUICallback('updateReputation', function(data, cb)
  lib.callback.register('lenix_bossdesk:server:updateReputation', false, function(success, message)
    if success then
...
RegisterNUICallback('transferFunds', function(data, cb)
  lib.callback.register('lenix_bossdesk:server:transferFunds', false, function(success, message)
    if success then
...
RegisterNUICallback('manageFunds', function(data, cb)
  lib.callback.register('lenix_bossdesk:server:manageFunds', false, function(success, message)
    if success then
...
RegisterNetEvent('lenix_bossdesk:client:openDesk', openDesk)
