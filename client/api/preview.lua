...
local function createPreviewCam(key, netId)
  ...
  camHandle = CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", cam.coords[1], cam.coords[2], cam.coords[3], cam.rotation.verticalrotate, cam.rotation.horizontalrotate, cam.rotation.left_n_right, cam.fov, false, 0)
...
local function clearPreviewCam(netId)
  ...
  if not success then tr_lib.trace('failed to destroy the preview vehicle') end
...
local function previewVehicle(key, index)
  ...
  if not isInPreview then
    local _, isPreviewSessionBusy = tr_lib.triggerPromise('sessionStatus')
